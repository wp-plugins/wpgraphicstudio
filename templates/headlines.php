...
$up_path2 = ''.$upload_path.'/'. $user_id .'/headlines';
$up_path5 = ''.$upload_path.'/'. $user_id .'/headlines/thumbs';
...
function emailGraphic($user_id) {
	...
	global $wpdb;
	   ...
	   $attachments = array( WP_CONTENT_DIR . '/uploads/wpgs/'.$user_id.'/headlines/'. $_POST['file'] .'' );
...
.isa_success {
    ...
    background-image:url('<?php echo $fileurl ?>/wp-content/uploads/wpgs/images/headlines/success.png');
}
</style>
<div class="isa_success">Your Headline Graphic Has Been eMailed to the eMail address on file.</div>
<?php
}
}
...
$myFile = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/headlines/'. $_POST['file'] .'.png';
$myThumb = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/headlines/thumbs/THUMB_'. $_POST['file'] .'';
...
<div class="isa_success">Your Headline Graphic Has Been Deleted From Your Gallery Below</div>
<?php }
...
imagejpeg($im, ''.$upload_path.'/'. $user_id .'/headlines/'. $newName .'.'. $_POST['format'] .'');
$file = ''.$upload_path.'/'. $user_id .'/headlines/'. $newName .'.'. $_POST['format'] .''; // (physical path)
chmod(''.$upload_path.'/'. $user_id .'/headlines/'. $newName .'.'. $_POST['format'] .'', 0777);
}
if (($_POST['format'] == 'png') && ($_POST['format'] != '')) {
imagepng($im, ''.$upload_path.'/'. $user_id .'/headlines/'. $newName .'.'. $_POST['format'] .'');
$file = ''.$upload_path.'/'. $user_id .'/headlines/'. $newName .'.'. $_POST['format'] .''; // (physical path)
chmod(''.$upload_path.'/'. $user_id .'/headlines/'. $newName .'.'. $_POST['format'] .'', 0777);
}

$fileurl = get_option( 'siteurl' );
$file = ''.$fileurl.'/wp-content/uploads/wpgs/'. $user_id .'/headlines/'. $newName .'.'. $_POST['format'] .''; // (physical path)
$filePath = ''.$fileurl.'/wp-content/uploads/wpgs/'. $user_id .'/headlines/'. $newName .'.'. $_POST['format'] .''; // (physical path)
...
if ($_POST['format'] == 'jpg') {
    imagejpeg($thumb, ''.$upload_path.'/'. $user_id .'/headlines/thumbs/THUMB_'. $newName .'.'. $_POST['format'] .'');
}
if ($_POST['format'] == 'png') {
    imagepng($thumb, ''.$upload_path.'/'. $user_id .'/headlines/thumbs/THUMB_'. $newName .'.'. $_POST['format'] .'');
    }
imagedestroy($im);
?>
<style type="text/css">
...
Your created Headline Has Been Saved To Your Headlines Graphic Gallery For Future Use.
<br>Click on your created Headline below to save to your computer....<br>
<b>Close This Window When Download Has Completed To Return To The Headlines Module.</b>
</div>
...
$full_url = ''.$fileurl.'/wp-content/uploads/wpgs/'.$user_id.'/headlines/';
$wp_upload_dir = wp_upload_dir();
$full_path = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/headlines/';
...
print "<p><b>Page:</b>\n";
	...
	for($i=1;$i<=$num_of_pages;$i++)
	{
		...
		{
		print "<a href=\"/graphic-gallery/headlines/2/?$i\">$i</a>\n";
		}
	}
...
print "<div style=\"float:left;\" align=\"center\"><form action=\"/headlines\" id=\"Delete\" method=\"post\"><input type=\"hidden\" name=\"file\" value=\"$file_name\"><input type=\"hidden\" name=\"view\" value=\"gallery\"><input type=\"hidden\" name=\"delete\" value=\"1\"><a href=\"javascript:;\" onclick=\"javascript:
...
print "<div style=\"float:left;\" align=\"center\"><form action=\"/headlines\" id=\"eMail\" method=\"post\"><input type=\"hidden\" name=\"file\" value=\"$file_name\"><input type=\"hidden\" name=\"view\" value=\"gallery\"><input type=\"hidden\" name=\"email\" value=\"1\"><a href=\"javascript:;\" onclick=\"javascript:
document.getElementById('eMail').submit()\" style=\"color:#494c4d; text-decoration:none;\"><strong>Email</strong></a></form></div><br>";
...
?><div margin = "0" align = "center" height = "775px" width = "900px">
<object id="flashcontent" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="900px" height="775px">
<?php echo'<param name="movie" value="'.plugins_url( '' ).'/wpgs-headlines/headlines.swf?logoURL='.$logo.'&NavColor='.$nav_hex.'" />'; ?>
  <!--[if !IE]>-->
  <?php echo '<object type="application/x-shockwave-flash" data="'.plugins_url( '' ).'/wpgs-headlines/headlines.swf?logoURL='.$logo.'&NavColor='.$nav_hex.'" width="900px" height="775px">'; ?>
  <!--<![endif]-->
  </div>
    <p>
