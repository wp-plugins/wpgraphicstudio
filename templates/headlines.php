...
$up_path2 = ''.$upload_path.'/wpgs/'. $user_id .'/headlines';
$up_path5 = ''.$upload_path.'/wpgs/'. $user_id .'/headlines/thumbs';
...
function emailGraphic($user_id) {
	...
	$emailFile = sanitize_file_name( $_POST['file'] );
	   ...
	   $attachments = array( $upload_path.'/wpgs/'.$user_id.'/headlines/'.$emailFile.'' );
...
$myFile = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/headlines/'. $deleteFile .'';
$myThumb = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/headlines/thumbs/THUMB_'. $deleteFile .'';
...
imagejpeg($im, ''.$upload_path.'/'. $user_id .'/headlines/'. $newName .'.'. $format .'');
$file = ''.$upload_path.'/'. $user_id .'/headlines/'. $newName .'.'. $format .''; // (physical path)
chmod(''.$upload_path.'/'. $user_id .'/headlines/'. $newName .'.'. $format .'', 0777);
}
if((isset($_POST['format'])) && ($_POST['format'] == 'png')) {
	$format = sanitize_file_name( $_POST['format'] );
imagepng($im, ''.$upload_path.'/'. $user_id .'/headlines/'. $newName .'.'. $format .'');
$file = ''.$upload_path.'/'. $user_id .'/headlines/'. $newName .'.'. $format .''; // (physical path)
chmod(''.$upload_path.'/'. $user_id .'/headlines/'. $newName .'.'. $format .'', 0777);
}
...
$file = ''.$fileurl.'/'. $user_id .'/headlines/'. $newName .'.'. $format .''; // (physical path)
$filePath = ''.$fileurl.'/'. $user_id .'/headlines/'. $newName .'.'. $format .''; // (physical path)
...
if ($format == 'jpg') {
    imagejpeg($thumb, ''.$upload_path.'/'. $user_id .'/headlines/thumbs/THUMB_'. $newName .'.'. $format .'');
}
if ($format == 'png') {
    imagepng($thumb, ''.$upload_path.'/'. $user_id .'/headlines/thumbs/THUMB_'. $newName .'.'. $format .'');
    }
imagedestroy($im);
?>
<style type="text/css">
...
$full_url = $wp_upload_dir['baseurl'] . '/wpgs/'.$user_id.'/headlines/';
$full_path = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/headlines/';

$cssurl = plugins_url( 'assets/css/style.css', dirname(__FILE__) );
...
<form action=\"/headlines\" id=\"Delete\" method=\"post\">
...
print "<form action=\"/headlines\" id=\"eMail\" method=\"post\">
<input type=\"hidden\" name=\"file\" value=\"$file_name\">
<input type=\"hidden\" name=\"view\" value=\"gallery\">
...
<?php echo'<param name="movie" value="'.plugins_url( 'headlines.swf', dirname(__FILE__) ).'?logoURL='.$logo.'&NavColor='.$decHex.'" />'; ?>
  <!--[if !IE]>-->
  <?php echo '<object type="application/x-shockwave-flash" data="'.plugins_url( 'headlines.swf', dirname(__FILE__) ).'?logoURL='.$logo.'&NavColor='.$decHex.'" width="900px" height="775px">'; ?>
  <!--<![endif]-->
  </div>
  <!--[if !IE]>-->
  </object>
