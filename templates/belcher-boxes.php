...
$up_path2 = ''.$upload_path.'/wpgs/'. $user_id .'/belcher-boxes';
$up_path5 = ''.$upload_path.'/wpgs/'. $user_id .'/belcher-boxes/thumbs';
...
$myFile = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/belcher-boxes/'. $deleteFile .'';
$myThumb = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/belcher-boxes/thumbs/THUMB_'. $deleteFile .'';
...
imagejpeg($im, ''.$upload_path.'/'. $user_id .'/belcher-boxes/'. $newName .'.'. $format .'');
$file = ''.$upload_path.'/'. $user_id .'/belcher-boxes/'. $newName .'.'. $format .''; // (physical path)
chmod(''.$upload_path.'/'. $user_id .'/belcher-boxes/'. $newName .'.'. $format .'', 0777);
}
if((isset($_POST['format'])) && ($_POST['format'] == 'png')) {
	$format = sanitize_file_name( $_POST['format'] );
imagepng($im, ''.$upload_path.'/'. $user_id .'/belcher-boxes/'. $newName .'.'. $format .'');
$file = ''.$upload_path.'/'. $user_id .'/belcher-boxes/'. $newName .'.'. $format .''; // (physical path)
chmod(''.$upload_path.'/'. $user_id .'/belcher-boxes/'. $newName .'.'. $format .'', 0777);
}
...
$file = ''.$fileurl.'/'. $user_id .'/belcher-boxes/'. $newName .'.'. $format .''; // (physical path)
$filePath = ''.$fileurl.'/'. $user_id .'/belcher-boxes/'. $newName .'.'. $format .''; // (physical path)
...
if ($format == 'jpg') {
    imagejpeg($thumb, ''.$upload_path.'/'. $user_id .'/belcher-boxes/thumbs/THUMB_'. $newName .'.'. $format .'');
}
if ($format == 'png') {
...
$full_url = $wp_upload_dir['baseurl'] . '/wpgs/'.$user_id.'/belcher-boxes/';
...
<form action=\"/belcher-boxes\" id=\"Delete\" method=\"post\">
...
<form action=\"/belcher-boxes\" id=\"eMail\" method=\"post\">
<input type=\"hidden\" name=\"file\" value=\"$file_name\">
<input type=\"hidden\" name=\"view\" value=\"gallery\">
...
<?php echo'<param name="movie" value="'.plugins_url( 'belcher-boxes.swf', dirname(__FILE__) ).'?logoURL='.$logo.'&NavColor='.$nav_hex.'" />'; ?>
  <!--[if !IE]>-->
  <?php echo '<object type="application/x-shockwave-flash" data="'.plugins_url( 'belcher-boxes.swf', dirname(__FILE__) ).'?logoURL='.$logo.'&NavColor='.$nav_hex.'" width="950px" height="450px">'; ?>
  <!--<![endif]-->
  </div>
    <p>
