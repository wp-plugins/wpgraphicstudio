...
function emailGraphic($user_id) {
	...
	global $wpdb;
	   ...
	   $attachments = array( WP_CONTENT_DIR . '/uploads/wpgs/'.$user_id.'/cta-boxes/'. $_POST['file'] .'' );
...
.isa_success {
    ...
    background-image:url('<?php echo $fileurl ?>/wp-content/uploads/wpgs/images/cta-boxes/success.png');
}
</style>
<div class="isa_success">Your Call To Action Box Graphic Has Been eMailed to the eMail address on file.</div>
<?php
}
}
...
$myFile = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/cta-boxes/'. $_POST['file'] .'.png';
$myThumb = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/cta-boxes/thumbs/THUMB_'. $_POST['file'] .'';
...
<div class="isa_success">Your Call To Action Box Graphic Has Been Deleted From Your Gallery Below</div>
<?php }
...
imagejpeg($im, ''.$upload_path.'/'. $user_id .'/cta-boxes/'. $newName .'.'. $_POST['format'] .'');
$file = ''.$upload_path.'/'. $user_id .'/cta-boxes/'. $newName .'.'. $_POST['format'] .''; // (physical path)
chmod(''.$upload_path.'/'. $user_id .'/cta-boxes/'. $newName .'.'. $_POST['format'] .'', 0777);
}
if (($_POST['format'] == 'png') && ($_POST['format'] != '')) {
imagepng($im, ''.$upload_path.'/'. $user_id .'/cta-boxes/'. $newName .'.'. $_POST['format'] .'');
$file = ''.$upload_path.'/'. $user_id .'/cta-boxes/'. $newName .'.'. $_POST['format'] .''; // (physical path)
chmod(''.$upload_path.'/'. $user_id .'/cta-boxes/'. $newName .'.'. $_POST['format'] .'', 0777);
}

$fileurl = get_option( 'siteurl' );
$file = ''.$fileurl.'/wp-content/uploads/wpgs/'. $user_id .'/cta-boxes/'. $newName .'.'. $_POST['format'] .''; // (physical path)
$filePath = ''.$fileurl.'/wp-content/uploads/wpgs/'. $user_id .'/cta-boxes/'. $newName .'.'. $_POST['format'] .''; // (physical path)
...
if ($_POST['format'] == 'jpg') {
    imagejpeg($thumb, ''.$upload_path.'/'. $user_id .'/cta-boxes/thumbs/THUMB_'. $newName .'.'. $_POST['format'] .'');
}
if ($_POST['format'] == 'png') {
    imagepng($thumb, ''.$upload_path.'/'. $user_id .'/cta-boxes/thumbs/THUMB_'. $newName .'.'. $_POST['format'] .'');
    }
imagedestroy($im);
?>
<style type="text/css">
...
Your created call to action box Has Been Saved To Your call to action boxes Graphic Gallery For Future Use.
<br>Click on your created call to action box below to save to your computer....<br>
<b>Close This Window When Download Has Completed To Return To The call to action boxes Module.</b>
</div>
...
<?php echo'<param name="movie" value="'.plugins_url( '' ).'/cta-boxes/cta-boxes.swf?logoURL='.$logo.'&NavColor='.$nav_hex.'" />'; ?>
  <!--[if !IE]>-->
  <?php echo '<object type="application/x-shockwave-flash" data="'.plugins_url( '' ).'/cta-boxes/cta-boxes.swf?logoURL='.$logo.'&NavColor='.$nav_hex.'" width="825px" height="525px">'; ?>
  <!--<![endif]-->
  </div>
    <p>
...
$full_url = ''.$fileurl.'/wp-content/uploads/wpgs/'.$user_id.'/cta-boxes/';
$wp_upload_dir = wp_upload_dir();
...
print "<div style=\"float:left;\" align=\"center\"><form action=\"/cta-boxes\" id=\"Delete\" method=\"post\"><input type=\"hidden\" name=\"file\" value=\"$file_name\"><input type=\"hidden\" name=\"view\" value=\"gallery\"><input type=\"hidden\" name=\"delete\" value=\"1\"><a href=\"javascript:;\" onclick=\"javascript:
...
print "<div style=\"float:left;\" align=\"center\"><form action=\"/cta-boxes\" id=\"eMail\" method=\"post\"><input type=\"hidden\" name=\"file\" value=\"$file_name\"><input type=\"hidden\" name=\"view\" value=\"gallery\"><input type=\"hidden\" name=\"email\" value=\"1\"><a href=\"javascript:;\" onclick=\"javascript:
