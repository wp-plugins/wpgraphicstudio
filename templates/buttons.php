...
function emailGraphic($user_id) {
	...
	$emailFile = sanitize_file_name( $_POST['file'] );
	   ...
	   $attachments = array( $upload_path.'/wpgs/'.$user_id.'/buttons/'.$emailFile.'' );
...
<div class="isa_success">Your Button Graphic Has Been eMailed to the eMail address on file.</div>
<?php
}
}
...
$myFile = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/buttons/'. $deleteFile .'';
$myThumb = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/buttons/thumbs/THUMB_'. $deleteFile .'';
...
<div class="isa_success">Your Button Graphic Has Been Deleted From Your Gallery Below</div>
<?php }
...
imagejpeg($im, ''.$upload_path.'/'. $user_id .'/buttons/'. $newName .'.'. $format .'');
$file = ''.$upload_path.'/'. $user_id .'/buttons/'. $newName .'.'. $format .''; // (physical path)
chmod(''.$upload_path.'/'. $user_id .'/buttons/'. $newName .'.'. $format .'', 0777);
}
if((isset($_POST['format'])) && ($_POST['format'] == 'png')) {
	$format = sanitize_file_name( $_POST['format'] );
imagepng($im, ''.$upload_path.'/'. $user_id .'/buttons/'. $newName .'.'. $format .'');
$file = ''.$upload_path.'/'. $user_id .'/buttons/'. $newName .'.'. $format .''; // (physical path)
chmod(''.$upload_path.'/'. $user_id .'/buttons/'. $newName .'.'. $format .'', 0777);
}
...
$file = ''.$fileurl.'/'. $user_id .'/buttons/'. $newName .'.'. $format .''; // (physical path)
$filePath = ''.$fileurl.'/'. $user_id .'/buttons/'. $newName .'.'. $format .''; // (physical path)
...
if ($format == 'jpg') {
    imagejpeg($thumb, ''.$upload_path.'/'. $user_id .'/buttons/thumbs/THUMB_'. $newName .'.'. $format .'');
}
if ($format == 'png') {
    imagepng($thumb, ''.$upload_path.'/'. $user_id .'/buttons/thumbs/THUMB_'. $newName .'.'. $format .'');
    }
imagedestroy($im);
?>
<style type="text/css">
...
$full_url = $wp_upload_dir['baseurl'] . '/wpgs/'.$user_id.'/buttons/';
...
<form action=\"/buttons\" id=\"Delete\" method=\"post\">
...
?><div margin = "0" align = "center" height = "450px" width = "850px">
<object id="flashcontent" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="850px" height="450px">
<?php echo'<param name="movie" value="'.plugins_url( 'buttons.swf', dirname(__FILE__) ).'?logoURL='.$logo.'&NavColor='.$nav_hex.'" />'; ?>
  <!--[if !IE]>-->
  <?php echo '<object type="application/x-shockwave-flash" data="'.plugins_url( 'buttons.swf', dirname(__FILE__) ).'?logoURL='.$logo.'&NavColor='.$nav_hex.'" width="850px" height="450px">'; ?>
  <!--<![endif]-->
  </div>
    <p>
