...
$up_path2 = ''.$upload_path.'/wpgs/'. $user_id .'/web-boxes';
$up_path5 = ''.$upload_path.'/wpgs/'. $user_id .'/web-boxes/thumbs';
...
$myFile = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/web-boxes/'. $deleteFile .'';
$myThumb = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/web-boxes/thumbs/THUMB_'. $deleteFile .'';
...
imagejpeg($im, ''.$upload_path.'/'. $user_id .'/web-boxes/'. $newName .'.'. $format .'');
$file = ''.$upload_path.'/'. $user_id .'/web-boxes/'. $newName .'.'. $format .''; // (physical path)
chmod(''.$upload_path.'/'. $user_id .'/web-boxes/'. $newName .'.'. $format .'', 0777);
}
if((isset($_POST['format'])) && ($_POST['format'] == 'png')) {
	$format = sanitize_file_name( $_POST['format'] );
imagepng($im, ''.$upload_path.'/'. $user_id .'/web-boxes/'. $newName .'.'. $format .'');
$file = ''.$upload_path.'/'. $user_id .'/web-boxes/'. $newName .'.'. $format .''; // (physical path)
chmod(''.$upload_path.'/'. $user_id .'/web-boxes/'. $newName .'.'. $format .'', 0777);
}
...
$file = ''.$fileurl.'/'. $user_id .'/web-boxes/'. $newName .'.'. $format .''; // (physical path)
$filePath = ''.$fileurl.'/'. $user_id .'/web-boxes/'. $newName .'.'. $format .''; // (physical path)
...
if ($format == 'jpg') {
    imagejpeg($thumb, ''.$upload_path.'/'. $user_id .'/web-boxes/thumbs/THUMB_'. $newName .'.'. $format .'');
}
if ($format == 'png') {
...
$full_url = $wp_upload_dir['baseurl'] . '/wpgs/'.$user_id.'/web-boxes/';
...
<form action=\"/web-boxes\" id=\"Delete\" method=\"post\">
...
<form action=\"/web-boxes\" id=\"eMail\" method=\"post\">
<input type=\"hidden\" name=\"file\" value=\"$file_name\">
<input type=\"hidden\" name=\"view\" value=\"gallery\">
...
?><div margin = "0" align = "center" height = "450px" width = "800px">
<object id="flashcontent" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="800px" height="450px">
<?php echo'<param name="movie" value="'.plugins_url( 'web-boxes.swf', dirname(__FILE__) ).'?logoURL='.$logo.'&NavColor='.$nav_hex.'" />'; ?>
  <!--[if !IE]>-->
  <?php echo '<object type="application/x-shockwave-flash" data="'.plugins_url( 'web-boxes.swf', dirname(__FILE__) ).'?logoURL='.$logo.'&NavColor='.$nav_hex.'" width="800px" height="450px">'; ?>
  <!--<![endif]-->
  </div>
    <p>
