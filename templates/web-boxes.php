...
$up_path2 = ''.$upload_path.'/'. $user_id .'/web-boxes';
$up_path5 = ''.$upload_path.'/'. $user_id .'/web-boxes/thumbs';
...
function emailGraphic($user_id) {
	...
	global $wpdb;
	   ...
	   $attachments = array( WP_CONTENT_DIR . '/uploads/wpgs/'.$user_id.'/web-boxes/'. $_POST['file'] .'' );
...
.isa_success {
    ...
    background-image:url('<?php echo $fileurl ?>/wp-content/uploads/wpgs/images/web-boxes/success.png');
}
</style>
...
$myFile = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/web-boxes/'. $_POST['file'] .'';
$myThumb = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/web-boxes/thumbs/THUMB_'. $_POST['file'] .'';
...
imagejpeg($im, ''.$upload_path.'/'. $user_id .'/web-boxes/'. $newName .'.'. $_POST['format'] .'');
$file = ''.$upload_path.'/'. $user_id .'/web-boxes/'. $newName .'.'. $_POST['format'] .''; // (physical path)
chmod(''.$upload_path.'/'. $user_id .'/web-boxes/'. $newName .'.'. $_POST['format'] .'', 0777);
}
if (($_POST['format'] == 'png') && ($_POST['format'] != '')) {
imagepng($im, ''.$upload_path.'/'. $user_id .'/web-boxes/'. $newName .'.'. $_POST['format'] .'');
$file = ''.$upload_path.'/'. $user_id .'/web-boxes/'. $newName .'.'. $_POST['format'] .''; // (physical path)
chmod(''.$upload_path.'/'. $user_id .'/web-boxes/'. $newName .'.'. $_POST['format'] .'', 0777);
}

$fileurl = get_option( 'siteurl' );
$file = ''.$fileurl.'/wp-content/uploads/wpgs/'. $user_id .'/web-boxes/'. $newName .'.'. $_POST['format'] .''; // (physical path)
$filePath = ''.$fileurl.'/wp-content/uploads/wpgs/'. $user_id .'/web-boxes/'. $newName .'.'. $_POST['format'] .''; // (physical path)
...
if ($_POST['format'] == 'jpg') {
    imagejpeg($thumb, ''.$upload_path.'/'. $user_id .'/web-boxes/thumbs/THUMB_'. $newName .'.'. $_POST['format'] .'');
}
if ($_POST['format'] == 'png') {
    imagepng($thumb, ''.$upload_path.'/'. $user_id .'/web-boxes/thumbs/THUMB_'. $newName .'.'. $_POST['format'] .'');
    }
imagedestroy($im);
?>
<style type="text/css">
...
$full_url = ''.$fileurl.'/wp-content/uploads/wpgs/'.$user_id.'/web-boxes/';
$wp_upload_dir = wp_upload_dir();
$full_path = $wp_upload_dir['basedir'] . '/wpgs/'. $user_id .'/web-boxes/';
...
print "<p><b>Page:</b>\n";
	...
	for($i=1;$i<=$num_of_pages;$i++)
	{
		...
		{
		print "<a href=\"/graphic-gallery/web-boxes/2/?$i\">$i</a>\n";
		}
	}
...
print "<div style=\"float:left;\" align=\"center\"><form action=\"/web-boxes\" id=\"Delete\" method=\"post\"><input type=\"hidden\" name=\"file\" value=\"$file_name\"><input type=\"hidden\" name=\"view\" value=\"gallery\"><input type=\"hidden\" name=\"delete\" value=\"1\"><a href=\"javascript:;\" onclick=\"javascript:
...
print "<div style=\"float:left;\" align=\"center\"><form action=\"/web-boxes\" id=\"eMail\" method=\"post\"><input type=\"hidden\" name=\"file\" value=\"$file_name\"><input type=\"hidden\" name=\"view\" value=\"gallery\"><input type=\"hidden\" name=\"email\" value=\"1\"><a href=\"javascript:;\" onclick=\"javascript:
...
?><div margin = "0" align = "center" height = "375px" width = "800px">
<object id="flashcontent" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="800px" height="450px">
<?php echo'<param name="movie" value="'.plugins_url( '' ).'/web-boxes/web-boxes.swf?logoURL='.$logo.'&NavColor='.$nav_hex.'" />'; ?>
  <!--[if !IE]>-->
  <?php echo '<object type="application/x-shockwave-flash" data="'.plugins_url( '' ).'/web-boxes/web-boxes.swf?logoURL='.$logo.'&NavColor='.$nav_hex.'" width="800px" height="450px">'; ?>
  <!--<![endif]-->
  </div>
    <p>
